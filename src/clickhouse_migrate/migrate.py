...
def migrations_to_apply(client: Client, incoming: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(terms_violated) > 0:
        raise AssertionError("Do not edit migrations once run, use migrations to correct older migrations")
    ...
    for _, row in migrations.iterrows():
        with open(row["script"]) as f:
            ...
            client.execute(
                ...
                [{"version": row["version"], "script": row["script"], "md5": row["md5"]}],
            )
    ...
    migrations = [
        {
            ...
            "md5": hashlib.md5(pathlib.Path(f"{migrations_home}/{f.name}").read_bytes()).hexdigest(),
        }
