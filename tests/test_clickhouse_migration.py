...
def test_should_migrate_in_queue_when_enabled(client):
    ...
    os.system(
        'gunzip < "tests/migrations_seq/test.csv.gz" | clickhouse-client --query="INSERT INTO pytest.sample FORMAT CSVWithNames"'
    )
    total_rows = 100000
    assert execute_and_inflate(client, "SELECT COUNT(*) AS nrow FROM pytest.sample").nrow.values[0] == total_rows
    enabled0 = execute_and_inflate(client, "SELECT COUNT(*) AS nrow FROM pytest.sample WHERE name > 3000").nrow.values[
        0
    ]
    ...
    assert (
        execute_and_inflate(client, "SELECT COUNT(*) AS nrow FROM pytest.sample WHERE enabled = 0").nrow.values[0]
        == enabled0
    )
    assert (
        execute_and_inflate(client, "SELECT COUNT(*) AS nrow FROM pytest.sample WHERE guard = 0").nrow.values[0]
        == enabled0
    )

    assert (
        execute_and_inflate(client, "SELECT COUNT(*) AS nrow FROM pytest.sample WHERE guard = 1").nrow.values[0]
        == total_rows - enabled0
    )
    assert (
        execute_and_inflate(client, "SELECT COUNT(*) AS nrow FROM pytest.sample WHERE guard = -1").nrow.values[0] == 0
    )
